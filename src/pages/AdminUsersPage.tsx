...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    fetchUsers();
  }, [filter.page, filter.size, filter.role, filter.active]);
        ...
        <div className="mb-6 rounded-xl bg-white p-6 shadow">
          ...
          <form onSubmit={handleSearch} className="flex flex-wrap items-end gap-4">
            <div className="min-w-[200px] flex-1">
              <label className="block text-sm font-medium text-gray-700">Поиск по имени</label>
              <input
                type="text"
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFilter((prev) => ({
                    ...prev,
                    role: e.target.value ? (e.target.value as UsersFilter['role']) : undefined,
                  }))
                }
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    <th className="px-6 py-3 font-medium text-gray-600">Имя пользователя</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {users.map((u) => (
                    <tr key={u.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center gap-1.5 text-xs font-medium ${u.active ? 'text-green-700' : 'text-red-700'}`}
                        >
                          <span
                            className={`h-1.5 w-1.5 rounded-full ${u.active ? 'bg-green-500' : 'bg-red-500'}`}
                          />
